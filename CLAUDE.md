...
Prod and dev Postgres are backed up off-site daily (plus a snapshot before every prod migration) to Cloudflare R2 + Backblaze B2, encrypted with `age`, GFS retention (7/4/12). Restores target a Supabase project (the `auth`/`storage`/`extensions` schemas are platform-managed). See `docs/runbook/13-backup-and-restore.md`, `.github/workflows/backup-db.yml`, and `.github/workflows/backup-verify.yml`.
...
- **Tenant role constraint.** `tenant_members.role` is `owner`-only (the `member` tier was removed in migration `20260429010000_owner_only_explicit_space_access`); space-level roles (`owner | editor | viewer`) live on `space_members`. There is no implicit tenant→space cascade: `has_space_access()` grants access only when an explicit `space_members` row exists (platform admin keeps a read-only support bypass). Tenant owners are backfilled as space owners on provisioning, but access itself is always the explicit row.
