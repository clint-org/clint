...
begin
  ...
  from (
    select distinct
           m.id, m.space_id, m.uploaded_by, m.is_sample, m.file_path, m.file_name,
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(r) order by r.uploaded_at desc), '[]'::jsonb)
    into v_rows
  from (
    select m.id, m.space_id, m.uploaded_by, m.is_sample, m.file_path, m.file_name,
           m.file_size_bytes, m.mime_type, m.material_type, m.title, m.uploaded_at,
           coalesce((
             select jsonb_agg(jsonb_build_object(
               'entity_type', l.entity_type,
...
-- Serve the new row shape immediately.
