-- Restore the preclinical-visibility guard in get_bullseye_assets.
--
-- The trial-filter recreation (20260618180000) -- and the company-logo
-- recreation before it (20260618170000) -- both rebuilt get_bullseye_assets
-- without the preclinical guard that 20260605140000 had restored. The result:
-- in a space that does NOT track preclinical, a preclinical-only asset leaks
-- back into the bullseye (it should drop out entirely). This is exactly what
-- integration/tests/preclinical-visibility.spec.ts asserts against, and it
-- regressed on prod.
--
-- This recreates get_bullseye_assets verbatim from 20260618180000 (keeping the
-- p_trial_ids filter and the company_logo_url payload) with two additive
-- changes, identical to the original guard from 20260605140000:
--   - v_show_preclin declared from public.space_shows_preclinical(p_space_id)
--   - the asset_phase rank CTE drops PRECLIN indications when the space does not
--     track preclinical: (v_show_preclin or ai.development_status is distinct
--     from 'PRECLIN'). An asset whose only indication is preclinical then has no
--     ranked phase and is excluded by the existing `having ... is not null`.
-- Signature / language / volatility / security / search_path are unchanged.
