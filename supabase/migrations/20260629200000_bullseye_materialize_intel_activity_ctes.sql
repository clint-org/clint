-- Migration: get_bullseye_assets -> materialize asset_intel + asset_activity CTEs.
--
-- Perf fix (no behavior change). get_bullseye_assets builds two per-asset CTEs,
-- asset_intel and asset_activity. Each is referenced once, so Postgres inlines
-- them; the planner then chose a nested-loop left join that re-evaluated the
-- entire asset_activity CTE once PER outer asset (N assets -> N*N inner runs).
-- The inner subqueries read primary_intelligence, whose RLS policy calls
-- has_space_access() + is_agency_member_of_space() per row, so those access
-- checks fired ~N*N times. On a 28-asset space this measured ~3.0s end-to-end.
--
-- Marking both CTEs MATERIALIZED forces each to compute exactly once, collapsing
-- the N*N evaluation to N. Measured on dev (same space, authenticated user, RLS
-- on): 3059ms -> 155ms (~20x). MATERIALIZED is a planner directive only; the
-- result rows are identical to the inlined form. Signature unchanged.
--
-- Base: exact copy of the live definition (20260629040400_bullseye_symmetric_
-- recent_upcoming.sql), verified byte-identical to the deployed dev function,
-- with the single change `as (` -> `as materialized (` on the two CTEs.
