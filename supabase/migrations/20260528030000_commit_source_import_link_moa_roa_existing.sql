-- Apply MOA/ROA assignments to existing matched assets, not just newly
-- created ones. Previously the linking block was inside the `else` branch
-- of the asset loop, so any MOA/ROA extracted for an asset the LLM matched
-- to existing inventory was silently dropped on commit.
--
-- Merge semantics: ON CONFLICT DO NOTHING preserves manually-curated
-- assignments and prevents duplicate junction rows.
...
begin
  ...
  if p_proposal->'assets' is not null then
    ...
    loop
      ...
      -- Apply MOA/ROA to BOTH new and existing assets. Merge semantics:
      -- ON CONFLICT DO NOTHING preserves anything already linked.
