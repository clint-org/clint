-- Repoint get_events_page_data + get_key_catalysts onto the unified public.events table.
--
-- The legacy marker schema and the old human-authored events table + its category table were
-- dropped in the event-model cutover. Both functions still named those tables in their plpgsql
-- bodies and errored at call time. This migration redefines them from their live bodies,
-- swapping only the source tables/columns onto the unified events schema. No output column name
-- or jsonb key changes: the frontend reads these as-is.
--
-- get_events_page_data: the two broken union legs (leg 1 old-events, leg 2 markers) are merged
-- into ONE leg reading public.events, with anchor-driven entity resolution. Leg 3
-- (trial_change_events) is unchanged. The surrounding CTE envelope (filtered/ranked/enriched,
-- search, sort, pagination, distribution, recent, final jsonb envelope) is byte-identical.
--
-- get_key_catalysts: the marker source collapses to events anchored to trials.
...
-- Data-conditional smoke: prove both repointed functions execute without a schema
-- error against the seeded demo space. Prod-safe (skips when the space is absent).
do $$
declare
...
begin
  if not exists (select 1 from public.spaces where id = v_space_id) then
    raise notice 'A2 smoke skipped: demo space % absent', v_space_id;
    return;
  end if;

  v_page := public.get_events_page_data(v_space_id);
  if v_page is null
     or not (v_page ? 'items')
     or not (v_page ? 'total')
     or not (v_page ? 'distribution')
     or not (v_page ? 'recent') then
    raise exception 'A2 smoke failed: get_events_page_data did not return the paginated envelope (got %)', v_page;
  end if;

  v_catalysts := public.get_key_catalysts(v_space_id, null, v_company_id, v_asset_id);
  if v_catalysts is null or jsonb_typeof(v_catalysts) <> 'array' then
    raise exception 'A2 smoke failed: get_key_catalysts did not return a jsonb array (got %)', v_catalysts;
  end if;

  raise notice 'A2 smoke ok: get_events_page_data total=%, get_key_catalysts rows=%',
    v_page ->> 'total', jsonb_array_length(v_catalysts);
end;
$$;
