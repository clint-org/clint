-- Add the linked entity's display name to each material link so the materials
-- UI can render a named, clickable chip ("[TRIAL] ATTAIN-1") instead of a
-- generic "Trial link". The name is resolved server-side per entity type:
--   trial   -> trials.acronym, falling back to trials.name
--   marker  -> markers.title
--   company -> companies.name
--   product -> assets.name        (entity_type 'product' is an asset)
--   space   -> spaces.name        (the engagement)
-- These are correlated lookups inside the existing SECURITY DEFINER list RPCs,
-- which already authorize space access before returning any row, so the lookups
-- add only the name and leak nothing new. No new function is introduced (the
-- CASE is inlined) so the API surface and capability mapping are unchanged.
