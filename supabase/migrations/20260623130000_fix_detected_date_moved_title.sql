-- Fix: detected `date_moved` feed rows rendered with a BLANK title.
--
-- The detected leg's title/priority CASEs read payload keys that the change
-- payloads never carry: `field` + `days_shifted` (the real keys are
-- `which_date` + `days_diff`). Because SQL `||` propagates NULL, a single
-- missing key collapsed the whole `'..' || .. || '..'` title to NULL, so these
-- rows showed only a date in the Events overview "Most recent" list, were
-- invisible to title search (`title ilike`), sorted blank, and never flagged
-- high priority on a >60-day shift.
--
-- This redefinition is the body from 20260618140000 verbatim EXCEPT the
-- detected-leg title and priority expressions, which now read the correct keys,
-- mirror the client summary (summaryFor in change-event-summary.ts: marker-side
-- event_date shifts lead with the marker title; direction is "pulled forward" /
-- "delayed"), and use concat_ws so a missing magnitude never blanks the label.
-- Signature unchanged; no other leg touched.
