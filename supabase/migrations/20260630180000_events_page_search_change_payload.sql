-- Activity page search fix (#192 follow-up): make the feed's free-text search
-- reach the detected-change PAYLOAD, not just the RPC-composed title.
--
-- For detected rows the visible Change column is composed client-side from
-- change_payload (drug names, arm names, intervention text, dates). The search
-- predicate only matched the server-side `title`, which for types like
-- intervention_changed / arm_added is just "Intervention Changed" -- so a query
-- for "tirzepatide" (rendered on screen from the payload) returned nothing. Add
-- `change_payload::text` to the search predicate so searching for what the user
-- SEES matches. The jsonb cast includes keys as well as values, an accepted
-- trade-off for a broad contains-search.
--
-- Same 18-arg signature as 20260630170000 (which established it), so this is a
-- plain CREATE OR REPLACE body swap -- no drop, no overload. Body is 170000's
-- verbatim with one added OR clause in the `filtered` CTE.
  ...
  filtered as (
    ...
    where (p_category_names is null or uf.category_name = any(p_category_names))
      ...
      and (
        ...
        -- Detected rows render their Change text from the payload (drug/arm
        -- names, intervention text). Search it so on-screen text is findable.
        or coalesce(uf.change_payload::text, '') ilike '%' || p_search || '%'
      )
  ),
  ranked as (
    select
...
-- Smoke: the payload-search predicate must run without error. Executing against
-- the zero space id (RLS-empty, no privilege raise) exercises the new
-- change_payload::text ilike clause alongside the source/type filters.
do $$
declare
  v jsonb;
begin
...
-- Body change only (same signature); reload keeps PostgREST's cache consistent.
