-- commit_source_import: skip markers/events that the AI matched to existing rows.
--
-- When a proposal marker or event carries match: {kind:'existing', id:<uuid>} and
-- that id is a live row in this space, we append the id to v_skipped_* and
-- continue (no duplicate insert). If the id is not valid in this space we fall
-- through and create as new (defensive). The returned jsonb gains a 'skipped'
-- key alongside 'created'.
--
-- Signature is unchanged; no PostgREST reload needed.
...
begin
  ...
  if p_proposal->'markers' is not null then
    ...
    loop
      -- Skip creation when the AI matched this marker to an existing row.
      v_match := v_item->'match';
      if v_match->>'kind' = 'existing' then
        if exists (
