-- migration: 20260628110000_events_model_qa_fixture
-- purpose: define seed_events_model_qa(p_space_id uuid), a deterministic,
--   idempotent SECURITY DEFINER seeder that populates one space with every
--   acceptance-matrix scenario required by Phase A and Phase C backtests.
--
-- Scenarios seeded (10 events + 1 brief with event-citation link):
--   Trial anchored : Trial Start, PCD, Topline (cited by brief), Approval
--   Asset anchored : Approval (high-sig), Distribution/hexagon (high-sig),
--                    projected Regulatory Filing (projection='primary', Q4 2026),
--                    hidden LOE Date (visibility='hidden', significance='high')
--   Company anchored: Strategic (visibility='pinned'), Leadership (feed-only)
--   Two companies + two assets (second pair for comparison-view stacking)
--   One published brief anchored to the trial, citing the Topline event
...
begin
  ...
  insert into public.trials
    (space_id, asset_id, name, status, phase, phase_type, study_type, created_by, display_order)
  values
    (p_space_id, v_asset1, 'QA Trial Alpha Phase 3', 'Active', 'Phase 3', 'P3', 'Interventional', v_uid, 1)
...
-- then tears everything down. Prod-safe (scratch entities only; no triggers
-- are disabled; events are deleted before the space cascade fires).
-- ============================================================================
do $$
declare
  v_scratch_uid    uuid;
...
begin
  ...
  raise notice 'SMOKE PASS: seed_events_model_qa % events, % pinned, % hidden, % projected, % companies, % brief',
