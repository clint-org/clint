-- Events feed: filter by category NAME, not category id.
--
-- The Events page overview histogram groups feed rows by category_name, a
-- display string that spans three taxonomies: event_categories (leg 1),
-- marker_categories (leg 2), and the synthetic detected-change categories
-- (leg 3, e.g. 'Catalyst lifecycle', 'Timeline'), which have category_id =
-- NULL. The id-based p_category_ids filter therefore cannot express a click on
-- a detected bucket, and two same-named categories (an event "Regulatory" and
-- a marker "Regulatory") have distinct ids the histogram has already merged.
--
-- A name filter applied AFTER the union covers all three legs uniformly and
-- matches the histogram's own grouping exactly. p_category_ids is retained
-- (still honored per-leg) for backward compatibility; the client now sends
-- p_category_names instead.
--
-- Body is the live definition (20260624120000 + 20260623130000 date_moved
-- title) copied verbatim; the ONLY changes are: the new p_category_names
-- parameter, its empty-array normalization, and one predicate in the `filtered`
-- CTE. Adding a parameter changes the function's argument list, so the old
-- 15-arg signature is dropped first (CREATE OR REPLACE would otherwise leave a
-- second overload and make PostgREST's call ambiguous).
...
-- Smoke: the new name filter must run without a type error (the bug this fixes
-- passed a category name into the uuid p_category_ids and raised
-- "invalid input syntax for type uuid"). Executing against an empty space id is
-- enough to exercise the signature + the new predicate.
do $$
declare
  v jsonb;
begin
  v := public.get_events_page_data(
    ...
    p_category_names := array['Regulatory', 'Catalyst lifecycle']
  );
