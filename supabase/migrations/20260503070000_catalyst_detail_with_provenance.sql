-- migration: 20260503070000_catalyst_detail_with_provenance
-- purpose: extend get_catalyst_detail to expose markers.metadata and the
--   parent trial's ctgov_last_synced_at, so the marker detail panel UI
--   can render the CT.gov provenance block (source field path, date type
--   actual / anticipated, last sync timestamp, link to clinicaltrials.gov)
--   for auto-derived markers without a separate round trip.
--
-- behavior:
--   - same shape as the prior 20260415180000 version, plus two new keys
--     in the inner v_catalyst object: 'metadata' (markers.metadata jsonb,
--     including {source: 'ctgov', field, snapshot_id, ctgov_date_type}
--     for auto-derived markers) and 'ctgov_last_synced_at' (from
--     trials.ctgov_last_synced_at, null for trials never synced).
--   - everything else is byte-for-byte identical to the prior version
--     so unrelated callers see no behavior change.
--
-- affected objects:
--   - replaced function: public.get_catalyst_detail(uuid)
