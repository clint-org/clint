-- =============================================================================
-- migration: surface companies.logo_url on the activity feed
--
-- Follow-on to 20260510120100_change_feed_product_company_marker_type. The
-- what-changed row component is replacing the ELI LILLY / ROCHE wordmark with
-- a logo tile (real PNG if companies.logo_url is set, deterministic monogram
-- fallback otherwise). Both get_activity_feed and get_trial_activity already
-- left-join companies, so this only adds one field per function. No filter,
-- ordering, or pagination semantics change.
-- =============================================================================
...
comment on function public.get_trial_activity(uuid, int) is
  'Recent change events for one trial; same row shape as get_activity_feed. Includes product + company name + logo, marker_color + marker_type_name, and from/to_marker_type_name for marker_reclassified rows. SECURITY INVOKER; raises 02000 if trial not found, 42501 if caller lacks has_space_access on the trial''s space.';
