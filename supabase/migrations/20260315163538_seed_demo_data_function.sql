-- function to seed demo data for the calling user
-- call via: select public.seed_demo_data();
-- idempotent: checks if user already has companies before inserting
...
begin
  ...
  select count(*) into existing_count from public.companies where user_id = uid;
