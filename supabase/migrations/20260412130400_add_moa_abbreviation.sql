-- migration: 20260412130400_add_moa_abbreviation
-- purpose: add abbreviation column to mechanisms_of_action (parallel to
--          therapeutic_areas.abbreviation and routes_of_administration.abbreviation),
--          and update seed_demo_data() to populate abbreviations in MOA inserts.
-- affected objects:
--   public.mechanisms_of_action (column added)
--   public.seed_demo_data (function replaced to include abbreviation in MOA inserts)

-- ---------------------------------------------------------------------------
-- 1. Schema change
-- ---------------------------------------------------------------------------
alter table public.mechanisms_of_action
  add column if not exists abbreviation text;

-- ---------------------------------------------------------------------------
-- 2. seed_demo_data: replace function to include MOA abbreviations
--    (only the mechanisms_of_action insert block changes vs 20260412130300;
--     all other blocks are carried forward verbatim)
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- ==========================================================================
  -- Mechanisms of action (with abbreviations)
  -- ==========================================================================
  insert into public.mechanisms_of_action (id, space_id, created_by, name, abbreviation, description, display_order) values
    (moa_sglt2,              p_space_id, uid, 'SGLT2 inhibitor',              'SGLT2i',       'Blocks sodium-glucose co-transporter 2 in the kidney.',               1),
    (moa_glp1,               p_space_id, uid, 'GLP-1 agonist',               'GLP-1 RA',     'Activates the GLP-1 receptor to increase insulin secretion.',          2),
    (moa_glp1_gip,           p_space_id, uid, 'GIP/GLP-1 dual agonist',      'GIP/GLP-1 RA', 'Dual activation of GIP and GLP-1 receptors.',                         3),
    (moa_sgc,                p_space_id, uid, 'sGC stimulator',              'sGC stim',     'Stimulates soluble guanylate cyclase to increase cGMP.',               4),
    (moa_ttr,                p_space_id, uid, 'TTR stabilizer',              'TTR stab',     'Stabilizes the transthyretin tetramer to prevent amyloid formation.',  5),
    (moa_nsmra,              p_space_id, uid, 'Non-steroidal MRA',           'ns-MRA',       'Non-steroidal mineralocorticoid receptor antagonist.',                 6),
    (moa_cardiac_myosin,     p_space_id, uid, 'Cardiac myosin modulator',    'CMM',          'Modulates cardiac myosin to improve contractility.',                   7),
    (moa_ns_investigational, p_space_id, uid, 'Investigational (undisclosed)','Inv',          'Early-stage asset, target not yet disclosed.',                        99);
...
comment on function public.seed_demo_data(uuid) is
  ...
  'Ozempic) and a representative set of mechanisms of action (with abbreviations) and routes '
  'of administration. Uses the new marker system schema: markers + marker_assignments + '
  'marker_notifications. Phase data is stored directly on the trial row (phase_type, '
  'phase_start_date, phase_end_date). Idempotent: returns early if the space already has companies.';
