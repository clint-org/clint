-- Fix #189: AI import over-precises fuzzy month dates.
--
-- The extraction pipeline now emits a per-event `date_precision` /
-- `end_date_precision` ('exact' | 'month' | 'quarter' | 'half' | 'year')
-- reflecting how precise the SOURCE actually was. commit_source_import
-- previously hard-coded 'exact' when calling create_event, so a month-only
-- phrase ("available in July") landed as a false exact day (July 1st).
--
-- This migration threads the proposal's precision through to create_event and
-- normalizes event_date / end_date to the period MIDPOINT that the rest of the
-- app expects for approximate dates (see marker-date-precision.ts
-- precisionMidpointISO: month -> the 15th, quarter -> 15th of the middle month,
-- half -> Apr 1 / Oct 1, year -> Jul 1). The label surfaces then render
-- "~Jul '26" instead of an implied exact day.
--
-- Reproduced verbatim from the live pg_get_functiondef (base:
-- 20260630120200_commit_source_import_event_indication) with the events-loop
-- create_event call as the only behavioural change, per the
-- CREATE-OR-REPLACE-from-live-def convention.
...
begin
  ...
  -- date_precision (#189): the worker emits how precise the source actually was.
  -- Thread it through and normalize the stored date to the period midpoint the
  -- rest of the app expects for approximate dates.
  if p_proposal->'events' is not null then
    ...
    loop
      ...
      declare
        ...
        -- #189: precision as stated by the source, and the raw dates before
        -- midpoint normalization.
        v_date_precision     text := coalesce(nullif(v_item->>'date_precision', ''), 'exact');
        v_end_date_precision text := coalesce(nullif(v_item->>'end_date_precision', ''), 'exact');
        v_raw_event_date     date := coalesce((v_item->>'event_date')::date, current_date);
        v_raw_end_date       date := (v_item->>'end_date')::date;
        v_event_date         date;
        v_end_date           date;
      begin
        ...
        -- Normalize approximate dates to the period MIDPOINT the app stores for
        -- fuzzy markers (marker-date-precision.ts precisionMidpointISO): month ->
        -- the 15th, quarter -> 15th of the middle month (Feb/May/Aug/Nov), half ->
        -- Apr 1 / Oct 1, year -> Jul 1. The specific day the LLM picked inside the
        -- period is discarded; the period is rendered from date_precision. 'exact'
        -- passes through unchanged.
        v_event_date := case v_date_precision
          when 'month'   then date_trunc('month', v_raw_event_date)::date + 14
          when 'quarter' then make_date(
                              extract(year from v_raw_event_date)::int,
                              (floor((extract(month from v_raw_event_date)::int - 1) / 3)::int) * 3 + 2,
                              15)
          when 'half'    then case when extract(month from v_raw_event_date)::int <= 6
                                then make_date(extract(year from v_raw_event_date)::int, 4, 1)
                                else make_date(extract(year from v_raw_event_date)::int, 10, 1)
                              end
          when 'year'    then make_date(extract(year from v_raw_event_date)::int, 7, 1)
          else v_raw_event_date
        end;

        if v_raw_end_date is null then
          v_end_date := null;
        else
          v_end_date := case v_end_date_precision
            when 'month'   then date_trunc('month', v_raw_end_date)::date + 14
            when 'quarter' then make_date(
                                extract(year from v_raw_end_date)::int,
                                (floor((extract(month from v_raw_end_date)::int - 1) / 3)::int) * 3 + 2,
                                15)
            when 'half'    then case when extract(month from v_raw_end_date)::int <= 6
                                  then make_date(extract(year from v_raw_end_date)::int, 4, 1)
                                  else make_date(extract(year from v_raw_end_date)::int, 10, 1)
                                end
            when 'year'    then make_date(extract(year from v_raw_end_date)::int, 7, 1)
            else v_raw_end_date
          end;
        end if;
