-- migration: 20260412130300_update_seed_demo_for_marker_redesign
-- purpose: rewrite public.seed_demo_data() to use the new marker system
--          schema (markers + marker_assignments + marker_notifications)
--          instead of the removed trial_phases and trial_markers tables.
--          Phase data is now stored directly on the trials row
--          (phase_type, phase_start_date, phase_end_date) -- one representative
--          phase per trial, using the highest/latest development phase.
--          The seed_pharma_demo function was already dropped by its own migration
--          (20260315192926) after its one-time execution; it cannot be called
--          again, so no replacement is required for it.
-- affected objects: public.seed_demo_data (function replaced)
-- notes: marker insertion pattern:
--          1. INSERT INTO public.markers  (space_id, marker_type_id, title,
--                                          projection, event_date, end_date,
--                                          created_by)
--          2. INSERT INTO public.marker_assignments (marker_id, trial_id)
--        is_projected is a GENERATED column -- never include in INSERT.
--        projection values: 'company' (projected), 'actual' (confirmed).
