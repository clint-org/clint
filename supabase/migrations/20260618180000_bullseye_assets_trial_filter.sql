-- Add a trial-ids filter to the bullseye asset RPC.
--
-- The shared landscape filter bar (Company / Asset / Trial / MOA / ROA / Phase /
-- Status / Category) drives all five landscape views. The Bullseye chart calls
-- get_bullseye_assets, which scoped by company / asset / indication / moa / roa /
-- phase but had no trial parameter, so picking a single trial in the shared bar
-- left the chart showing every asset instead of the asset(s) on that trial.
--
-- Assets link to trials through public.trial_assets (trial_id, asset_id). This
-- migration recreates get_bullseye_assets verbatim from its latest definition
-- (20260618170000) with one additive change: a new trailing parameter
-- `p_trial_ids uuid[] default null` plus its empty-array normalization and a
-- single candidate-asset scope clause that keeps only assets having at least one
-- of the named trials -- mirroring how the function already scopes by indication
-- / moa / roa via an `exists` subquery. Language / volatility / security /
-- search_path are preserved; the body delimiter stays `$$`. No new public
-- function is created. Other landscape RPCs are unchanged.
--
-- A defaulted trailing parameter makes the new signature distinct from the prior
-- 7-arg one, so both would coexist and a 7-arg call (the client's current shape)
-- becomes ambiguous. Drop the prior signature first; the recreated function below
-- keeps every existing parameter, so callers that pass the old args still resolve
-- to it via the defaulted p_trial_ids.
drop function if exists public.get_bullseye_assets(uuid, uuid[], uuid[], uuid[], uuid[], text[], uuid[]);
