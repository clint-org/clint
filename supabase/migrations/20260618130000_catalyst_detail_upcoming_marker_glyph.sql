-- Add the marker glyph fields (shape, color, inner mark, projection) to each
-- entry in get_catalyst_detail's upcoming_markers list, so the detail pane can
-- render the real marker icon per upcoming marker instead of a generic dot.
-- Recreated verbatim from 20260530140000 with only the v_upcoming block changed.
