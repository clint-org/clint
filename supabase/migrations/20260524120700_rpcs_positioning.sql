-- migration: 20260524120700_rpcs_positioning
-- purpose: rewrite get_positioning_data for indication model. scoping changes
--          from therapeutic area to indication, phase rollup reads from
...
begin
  ...
  asset_highest_phase as (
    ...
    where ai.development_status is not null
      and (p_indication_ids is null or ai.indication_id = any(p_indication_ids))
    group by ea.asset_id, ea.asset_name, ea.asset_generic_name, ea.company_id, ea.company_name
  ),

  asset_groups as (
    select
      ahp.asset_id, ahp.asset_name, ahp.asset_generic_name,
      ahp.company_id, ahp.company_name,
  ...
  return jsonb_build_object('grouping', p_grouping, 'count_unit', p_count_unit, 'bubbles', v_bubbles);
end;
$$;
