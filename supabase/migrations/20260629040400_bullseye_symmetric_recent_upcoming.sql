-- Migration: bullseye recent_markers -> symmetric Recent + Upcoming window.
-- The recent_markers subquery in get_bullseye_assets + get_bullseye_data previously took the
-- 3 events with the largest event_date (order by event_date desc limit 3). That skews to the
-- furthest-future events and starves the "Recent" (past) bucket the frontend derives in
-- deriveBullseyeEventBuckets, so a focused asset often showed an empty Recent list. It now
-- returns the 3 nearest PAST (event_date < current_date, desc) UNION ALL the 3 nearest FUTURE
-- (event_date >= current_date, asc) so both lists fill symmetrically. Anchor sourcing
-- (trial-anchored) and every other line are byte-identical to the live body. (Surfacing
-- asset/company-anchored events in the bullseye is a separate follow-up: asset-level events
-- carry no indication, so they do not map cleanly onto the indication-grouped spokes.)
