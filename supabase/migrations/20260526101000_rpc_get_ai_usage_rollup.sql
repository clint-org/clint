-- migration: 20260525101000_rpc_get_ai_usage_rollup
-- purpose: super-admin telemetry RPC. Three drill-down levels:
--          'platform' -> tenant rows, 'tenant' -> space rows, 'space' -> import rows.
...
revoke execute on function public.get_ai_usage_rollup(text, uuid, interval) from public;
grant execute on function public.get_ai_usage_rollup(text, uuid, interval) to authenticated;

comment on function public.get_ai_usage_rollup(text, uuid, interval) is
  'Platform-admin only. Three-level AI usage drill-down: platform -> tenant -> space -> individual imports.';
...
begin
  assert exists (
    select 1 from pg_proc
     where proname = 'get_ai_usage_rollup'
       and pronamespace = 'public'::regnamespace
  ), 'get_ai_usage_rollup function not found';

  raise notice 'smoke: get_ai_usage_rollup created OK';
end$$;
