...
describe('bullseye node PI badge', () => {
  it('renders the brand bookmark badge for nodes with intelligence', () => {
    ...
    // Agency-aware aria-label: "Has {Agency} intelligence" via the chart computed.
    expect(chartHtml).toContain('[attr.aria-label]="assetPiAriaLabel()"');
    expect(chartTs).toContain('intelligenceLabel()');
