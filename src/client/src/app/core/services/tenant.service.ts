import { inject, Injectable } from '@angular/core';

import { Tenant, TenantMember, TenantInvite } from '../models/tenant.model';
import { SupabaseService } from './supabase.service';

@Injectable({ providedIn: 'root' })
export class TenantService {
  private supabase = inject(SupabaseService);

  async listMyTenants(): Promise<Tenant[]> {
    const { data } = await this.supabase.client
      .from('tenants')
      .select('*')
      .order('created_at')
      .throwOnError();
    return data ?? [];
  }

  async getTenant(id: string): Promise<Tenant> {
    const { data } = await this.supabase.client
      .from('tenants')
      .select('*')
      .eq('id', id)
      .single()
      .throwOnError();
    return data;
  }

  async updateTenant(id: string, updates: Partial<Tenant>): Promise<Tenant> {
    const { data } = await this.supabase.client
      .from('tenants')
      .update(updates)
      .eq('id', id)
      .select()
      .single()
      .throwOnError();
    return data;
  }

  async deleteTenant(id: string): Promise<void> {
    await this.supabase.client.from('tenants').delete().eq('id', id).throwOnError();
  }

  async listMembers(tenantId: string): Promise<TenantMember[]> {
    const { data } = await this.supabase.client
      .rpc('list_tenant_members', { p_tenant_id: tenantId })
      .order('created_at')
      .throwOnError();
    return data ?? [];
  }

  async removeMember(tenantId: string, userId: string): Promise<void> {
    await this.supabase.client
      .from('tenant_members')
      .delete()
      .eq('tenant_id', tenantId)
      .eq('user_id', userId)
      .throwOnError();
  }

  /**
   * Add or invite a tenant owner. Calls add_tenant_owner RPC, which validates
   * the caller's permissions, enforces the agency.email_domain rule when set,
   * and either inserts a tenant_members(owner) row directly or holds a
   * tenant_invites row for code-based acceptance via accept_invite().
   */
  async addTenantOwner(
    tenantId: string,
    email: string
  ): Promise<{
    owner_invited: boolean;
    user_id?: string;
    invite_id?: string;
    invite_code?: string;
    email?: string;
  }> {
    const { data } = await this.supabase.client
      .rpc('add_tenant_owner', {
        p_tenant_id: tenantId,
        p_email: email,
      })
      .throwOnError();
    return data as {
      owner_invited: boolean;
      user_id?: string;
      invite_id?: string;
      invite_code?: string;
      email?: string;
    };
  }

  async listInvites(tenantId: string): Promise<TenantInvite[]> {
    const { data } = await this.supabase.client
      .from('tenant_invites')
      .select('*')
      .eq('tenant_id', tenantId)
      .is('accepted_at', null)
      .order('created_at')
      .throwOnError();
    return data ?? [];
  }

  async joinByCode(code: string): Promise<Tenant> {
    const { data } = await this.supabase.client
      .rpc('accept_invite', {
        p_code: code,
      })
      .throwOnError();
    return data as Tenant;
  }

  async uploadLogo(tenantId: string, file: File): Promise<string> {
    const ext = file.name.split('.').pop() ?? 'png';
    const path = `${tenantId}/logo.${ext}`;

    const { error: uploadError } = await this.supabase.client.storage
      .from('tenant-logos')
      .upload(path, file, { upsert: true, contentType: file.type });
    if (uploadError) throw uploadError;

    const { data } = this.supabase.client.storage.from('tenant-logos').getPublicUrl(path);

    return data.publicUrl;
  }

  async deleteLogo(tenantId: string): Promise<void> {
    const { data: files } = await this.supabase.client.storage.from('tenant-logos').list(tenantId);

    if (files && files.length > 0) {
      const paths = files.map((f) => `${tenantId}/${f.name}`);
      await this.supabase.client.storage.from('tenant-logos').remove(paths);
    }
  }

  async getTenantAccessSettings(
    tenantId: string
  ): Promise<{ email_domain_allowlist: string[]; email_self_join_enabled: boolean }> {
    const { data } = await this.supabase.client
      .rpc('get_tenant_access_settings', {
        p_tenant_id: tenantId,
      })
      .throwOnError();
    return data as { email_domain_allowlist: string[]; email_self_join_enabled: boolean };
  }

  async updateTenantAccess(
    tenantId: string,
    settings: { email_domain_allowlist?: string[]; email_self_join_enabled?: boolean }
  ): Promise<void> {
    await this.supabase.client
      .rpc('update_tenant_access', {
        p_tenant_id: tenantId,
        p_settings: settings,
      })
      .throwOnError();
  }

  async selfJoinTenant(subdomain: string): Promise<{ id: string; name: string; role: string }> {
    const { data } = await this.supabase.client
      .rpc('self_join_tenant', {
        p_subdomain: subdomain,
      })
      .throwOnError();
    return data as { id: string; name: string; role: string };
  }

  /**
   * Best-effort check: tenant_members RLS lets a member read their own
   * row + fellow members' rows. If we read 0 rows for a tenant, the caller
   * is not a member. Returns false on any error to be safe (the caller
   * will then attempt self-join, which is idempotent on conflict).
   */
  async checkIsTenantMember(tenantId: string): Promise<boolean> {
    const { data, error } = await this.supabase.client
      .from('tenant_members')
      .select('id')
      .eq('tenant_id', tenantId)
      .limit(1);
    if (error) return false;
    return (data ?? []).length > 0;
  }
}
