...
- MOA: extract the mechanism of action from intervention descriptions when stated (e.g., "GLP-1 receptor agonist", "PD-1 inhibitor", "SGLT2 inhibitor"). Use standard pharmacological class names. Prefer matching existing inventory mechanisms_of_action by exact name string. Only extract what the data supports; leave as empty array if the intervention description does not indicate a mechanism.
- ROA: extract the route of administration from intervention type and description when stated (e.g., "Oral", "Subcutaneous", "Intravenous"). Normalize to standard terms. Prefer matching existing inventory routes_of_administration by exact name string. If intervention type is DRUG with no route detail, leave as empty array rather than guessing.
