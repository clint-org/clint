...
Three layers, each scoped to one concern.

### Migration-level (psql inline smoke)

Append to the same migration that replaces `_materialize_trial_from_snapshot`, matching the pattern at the bottom of `20260503050000_derive_phase_type_from_ctgov.sql`:
...
- Materialize with a changed phase against a row with `phase_type = 'P2', source = 'ctgov'`: `phase_type` updates, source stays `'ctgov'`, **one** `phase_changed` event row appears with old/new in payload.
...
- Materialize against a row with `phase_type = 'P3', source = 'analyst'` while derive returns `'P2'` (post-backfill scenario, the override gets overwritten): updates to `'P2'`, source flips to `'ctgov'`, one event with `old_source: 'analyst'` in payload.
- Backfill smoke: seed three trials (matching, diverging, no-snapshot), assert the source columns land where expected.
...
### Component-level (Vitest, `trial-edit-dialog.spec.ts`)

- Mount with a trial where `phase_type_source = 'ctgov'`: Phase select is disabled, `[ct.gov]` badge present, tooltip shows.
- Mount with `phase_type = null, phase_type_source = null`: Phase select editable. Picking `P2` + saving calls service with `{ phase_type: 'P2' }` (service layer adds the source).
- Mount with mixed sources (phase ct.gov, dates null): phase locked, date pickers editable.

Per the *Pair tests with each task* memory: every task in the implementation plan includes its spec inline, not piled into a "Phase N: Tests" section.
