...
equal the vault secret `ctgov_worker_secret`. There is no detector for drift between
the two, so a mass-`42501` run is the symptom. To confirm the vault side:
...
1. **Watcher-on-default-branch promotion lag (process).** Observability and fixes
   ...
   check above.
2. **Secret drift has no detector and is catastrophic.** A `CTGOV_WORKER_SECRET` vs
   `ctgov_worker_secret` mismatch fails every ingest. Recommendation: add a periodic
   check (a tiny worker route or a GHA step) that performs one authenticated probe
   and alerts on `42501`, separate from the staleness check, so drift is named
