...
- id: engagement-landing-signal-strip
  summary: Single-row status line of four engagement motion signals (P3 readouts, catalysts, new intel, trial moves), each with metric value, label, and window-label, routing to the corresponding browse view. Warn items (counts > 0 that demand attention) carry an amber tint.
  routes:
    - /t/:tenantId/s/:spaceId
  rpcs:
    - get_space_landing_stats
