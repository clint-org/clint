...
- `help/markers` -- marker semantics (color rule, projection convention, full marker-type list). Live-rendered from `MarkerTypeService` so the type list always matches the space's actual configuration. Linked from the dashboard legend strip and the Settings > Taxonomies (Event Types tab) page.
