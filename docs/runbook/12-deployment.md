...
There is no demo-data path in the current product. Historically (pre-2026-04-30) the platform exposed three flavors: the `handle_new_user` trigger auto-provisioned Boehringer/Azurity tenants on signup (retired in migration 41); `provision_demo_workspace()` did the same on demand via `/provision-demo` (dropped in migration 78); and `seed_demo_data(p_space_id)` populated an empty space with companies/products/trials, called automatically by `landscape-state.service` whenever a space had zero companies (dropped in migration 81). All three were vestiges of the pre-firewall, pre-engagement-scope era. If demo data is needed for sales or marketing in the future it should be a separate explicit flow with `is_space_owner` permission gating, not an auto-trigger on empty.
