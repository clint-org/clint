...
- **Pulse header** (inline in `EngagementLandingComponent`): three-row panel. Row 1 is the identity strip: engagement name (`<h1>` mono uppercase) and an "Active since YYYY-Q#" subline. Row 2 is the adaptive brief: a terse one- or two-sentence plain-text summary derived from the motion stats (P3 readouts, catalysts, new intel, trial moves, LOE), visible only when the brief window is "open" and there is at least one non-zero signal; hidden otherwise. Row 3 is the motion strip: five stat tiles (p3 readouts 90d, catalysts 90d, new intel 7d, trial moves 30d, LOE 365d), each a router link to the corresponding browse page. Counts come from a single `get_space_landing_stats(p_space_id)` RPC call.
...
**Stats RPC.** `get_space_landing_stats(p_space_id uuid)` returns a single jsonb object. Inventory counts: `active_trials` (excludes recruitment_status in `completed`/`withdrawn`/`terminated`), `companies` (distinct company_id on products), `programs` (count of products), `intelligence_total` (total published rows, kept for backward compat). Motion signals: `catalysts_90d` (all markers within today + 90 days), `p3_readouts_90d` (Data-category markers on Phase 3 trials within 90 days), `new_intel_7d` (published primary intelligence within 7 days), `trial_moves_30d` (distinct trials with a phase transition or terminal status change within 30 days), `loe_365d` (Loss-of-Exclusivity markers within 365 days). Gated on `has_space_access`; security definer; language sql stable.
