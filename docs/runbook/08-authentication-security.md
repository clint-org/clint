...
`companies`, `assets`, `trials`, `trial_phases`, `trial_markers`, `trial_notes`, `indications`, `conditions`, `condition_indication_map`, `asset_indications`, `trial_conditions` use:
...
Write access on tenant child tables (companies, assets, trials, etc.) goes through `has_space_access(space_id, ['owner', 'editor'])` — agency *members* are not implicitly editors of child data, only agency *owners* are.
