...
- **Status:** done (commit a7c13f6). Verified live 2026-05-01 by aadi529 on `pfizer.clintapp.com` -- after the deploy, hitting Sync from CT.gov with NCT04520438 populated the trial-form fields and surfaced the success toast. Root cause was the `Content-Security-Policy` `connect-src` allowlist in `src/client/public/_headers` -- it included Supabase and Cloudflare Insights but not `clinicaltrials.gov`, so every browser blocked the fetch outright before any request left the page. Console showed `Refused to connect because it violates the document's Content Security Policy`. The CT.gov v2 API itself was fine; the mapping in `ctgov-sync.service.ts` was fine; the trial-form catch was fine. The fix is one entry: added `https://clinicaltrials.gov` to `connect-src`. Runbook CSP snippets in `docs/runbook/08-authentication-security.md` and `docs/runbook/12-deployment.md` updated to reflect the new policy and call out that the entry exists for the trial-form NCT sync path.
- **Sidenote on the "no error surfaced" detail in the original entry:** a CSP block does throw `TypeError: Failed to fetch`, which the trial-form catch translates into the in-form `<p-message severity="error">` banner at the top of the dialog. The original tester likely missed the banner (the visual focus is on the NCT input area). Not a code bug; no further work.
- **Surfaced:** 2026-05-01 retest of follow-up #5; novaelevatellc tried it while filling out a new trial and reported "nct sync isn't working at all". Diagnosed 2026-05-01 by aadi529 via DevTools console.

### 17. Form validation: required fields not enforced; submit allowed with empty NOT NULL columns; generic error
- **Status:** done (commit dfad1b9, runbook pattern documented in 293bfa9). Verified live 2026-05-01 by aadi529 across all four forms on `pfizer.clintapp.com` SGLT2 Pipeline: asterisks render correctly on every required field, Save buttons stay disabled until populated, and the symptomatic "save with empty Product / Therapeutic area" path is no longer reachable.
- **Implementation:** Cross-form sweep across trial-form, marker-form, product-form, note-form. Each required field now renders a visible asterisk via `[required]` on FormFieldComponent (or inline span on inline-template forms) and `aria-required="true"` on the underlying input. Save buttons disable until every required field is populated: trial-form blocks until name + product + therapeutic area are set, marker-form until category + marker type + title + event date + at-least-one-trial are set, product-form until name + company are set, note-form until content is set. New `core/util/db-error.ts` maps Postgres 23502 (NOT NULL) and 23503 (FK) errors back to the form's column->label map and surfaces "<Field> is required" in the existing in-form error banner; this is defense-in-depth for any future schema-added NOT NULL the client doesn't yet know about. FormActionsComponent gained a `[disabled]` input for the trial-form / product-form path; inline buttons in marker-form and note-form bind `[disabled]` directly. The previous silent `if (!this.name.trim()) return` guards became `if (!this.canSubmit) return`. Pattern is documented for future forms in `docs/runbook/11-developer-guide.md` under "Required fields and constraint errors".
- **Scope:** `src/client/src/app/features/manage/trials/trial-form.component.{ts,html}`, `marker-form.component.ts`, `note-form.component.ts`, `src/client/src/app/features/manage/products/product-form.component.{ts,html}`, plus shared `src/client/src/app/shared/components/form-actions.component.ts` and new `src/client/src/app/core/util/db-error.ts`.
...
- **Status:** done (commit 01c3c23, migration 20260501080000). Verified live 2026-05-01:
  - **DB layer (curl with aadimadala's JWT):** `tenant_members_view` returned `is_agency_backed: true` for aadi529 and `false` for aadimadala; the DELETE attempt against aadi529's row returned HTTP 403 with `42501 Cannot remove an agency owner from this tenant. Tenant access for this user is granted by the parent agency; contact a platform admin if eviction is needed.`
  - **UI (aadimadala's session):** the "via agency" tag renders next to aadi529's name and her row has no `...` actions menu. Other rows unchanged.
  - Platform-admin override path (skipped in verification because the side-effect is destructive) is preserved by the `not public.is_platform_admin()` clause in the trigger; verified by code reading.
  
  Design call: tenant clients cannot evict their parent agency from their own tenant -- it's a contractual matter that goes through a platform admin (option C with the option-A UI nuance). Implemented as both a UI gate AND a database guard so the rule holds whether the eviction is attempted from tenant settings, a stale tab, or curl.
- **Implementation:**
