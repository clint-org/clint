...
- `[-]` **P3.2 Hero trials drill into empty sections** ("No read yet", "0 reads", "No materials"). Demo-data, not code. Resolved operationally by regenerating the dev demo space (delete the space -> create a fresh one -> run the `seed-demo` flow, which calls `seed_demo_data(space_id)`): a clean seed restores the intended hero-trial depth and clears the UI-38/39/40 garbage in one pass. No surgical SQL or hand-authored backfill needed. *Grouped with UI-40.*
